...
function Clock() {
  ...
  this.getTime = function() {
    ...
    else if(adjustment < 0)
    {
      ...
      {
        this.adjustment += time - lastGetTime;
        time = lastGetTime;
      }
    }
...
function Client(identifierParameter, serverAddressParameter, serverPortParameter) {
  ...
  this.connect = function() {
    ...
    socket.on("ping request", function(messageString) {
      ...
      that._pingBack(message, messageString);
    });
  ...
  this._restartClock = function(restartClockMessage) {
    ...
    this.framerate = restartClockMessage.getFramerate();
  };
  ...
  this._updateTime = function(updateTimeMessage) {
    clock.adjust(updateTimeMessage.getDrift());
  };
  ...
  this._pingBack = function(message, messageString) {
    if(message.getClientId() == identifier)
    {
      socket.emit("ping response", messageString);
    }
  }
