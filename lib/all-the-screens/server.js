...
function Server(portParameter) {
  ...
  this.setFramesPerSecond = function(framesPerSecondParameter) {
    if(state != "inactive")
    {
      throw new Error("Cannot set framesPerSecond while the server is running");
    }
    framesPerSecond = framesPerSecondParameter;
    return true;
  };
  ...
  this.pingClient = function(clientId) {
    ...
    setTimeout(function() {
      ...
      {
        var lastPingResolveTime = clientPingRequests[pingRequestId].time;
        ...
        if(clientData.lastPingResolveTime === null || lastPingResolveTime > clientData.lastPingResolveTime)
        {
          clientData.lastPingResolveTime = lastPingResolveTime;
  ...
  this._getPingResponseFunction = function(that) {
    return function(messageObject) {
      ...
      if(clientPingRequests[pingRequestId] !== undefined)
      {
        ...
        lastPingResolveTime = clientPingRequests[pingRequestId].time;
        delete clientPingRequests[pingRequestId];
      }
  ...
  this._getRegisterClientMessageFunction = function(that, socket) {
    return function(messageObject, callback)
    {
      ...
      try
      {
        if(that._registerClient(message, socket) && callback !== undefined && callback !== null)
        {
